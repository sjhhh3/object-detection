...
CHINESE = {"person": "人", "chair": "椅子", "car":"车辆","bus":"长型车", "truck":"皮卡车", "motorcycle":"摩托车", "bicycle":"自行车", "dog":"狗", "traffic light":"交通灯"}
...
def draw_bounding_box_on_image(image,
  ...
  if use_normalized_coordinates:
    ...
    #showing = np.asarray(image)
    #cv2.imshow("screenshot", showing[int(top):int(bottom), int(left):int(right)])
      ...
      if scores is None:
        ...
        if not skip_labels:
          if not agnostic_mode:
            ...
            # Jason1加入中文显示，计数器
            if CHINESE.get(str(class_name)):
                display_str = CHINESE.get(str(class_name))
                counting[display_str] += 1
            else:
        ...
        if agnostic_mode:
          ...
          box_to_color_map[box] = STANDARD_COLORS[
              classes[i] % len(STANDARD_COLORS)+2]   #修改颜色
        
   # Jason3加入计数器 
  print([str(key)+ ':' + str(value) for key,value in counting.items()])
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  draw = ImageDraw.Draw(image_pil)
  font = ImageFont.truetype("simhei.ttf", 20, encoding="utf-8")
  draw.text((10,10),"计数："+ str([str(key)+ ':' + str(value) for key,value in counting.items()])[1:-1],fill="black",font=font)
  np.copyto(image, np.array(image_pil))
